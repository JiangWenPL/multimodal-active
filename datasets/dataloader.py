...
class HabitatDataScene(Dataset):
    ...
    def __init__(self, options, config_file, scene_id, slam_config, existing_episode_list=[]):
        ...
        if options.dataset_type == "mp3d":
            ...
            if slam_config.use_ep_start and \
                  os.path.exists(f"{options.root_path}/data/datasets/pointnav/mp3d/v1/train/content/{scene_id}.json.gz"):
        ...
        if options.dataset_type == "mp3d":
            if cfg.habitat.dataset.split == "train":
                ep_file_path = os.path.join(options.root_path, "data/datasets/pointnav/mp3d/v1", cfg.habitat.dataset.split, "content", scene_id + ".json.gz")
            else:
        ...
        print(set(i['scene_id'].split('/')[-1].split('.')[0] for i in self.data['episodes']))
        logging.info(f"Number of episodes in scene {self.scene_id}: {self.number_of_episodes}")
