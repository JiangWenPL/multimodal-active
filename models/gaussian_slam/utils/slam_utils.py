...
def get_loss_tracking(config, image, depth, opacity, viewpoint, initialization=False):
    ...
    if config["Training"]["monocular"]:
        return get_loss_tracking_rgb(config, image_ab, None, opacity, viewpoint)
...
def get_loss_tracking_rgb(config, image, depth, opacity, viewpoint):
    ...
    if depth is not None:
        depth_pixel_mask = (depth > 0.01).view(*mask_shape)
...
def get_loss_tracking_rgbd(
    ...
    # apply a center mask
    rotate_mask = torch.zeros((1, 800, 800), device=depth.device)
    rotate_mask[:, :, 120:680] = 1
    ...
    if depth is not None:
        depth_pixel_mask = (depth > 0.01).view(*mask_shape)
        rgb_pixel_mask = rgb_pixel_mask * depth_pixel_mask * rotate_mask
    l1_rgb = opacity * torch.abs(image * rgb_pixel_mask - gt_image * rgb_pixel_mask)

    depth_mask = depth_pixel_mask * opacity_mask * rotate_mask
...
def get_loss_mapping_rgbd(config, image, depth, viewpoint, initialization=False):
    ...
    l1_rgb = torch.abs(image - gt_image) # * rgb_pixel_mask
