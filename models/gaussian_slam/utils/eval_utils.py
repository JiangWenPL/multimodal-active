...
def evaluate_evo(poses_gt, poses_est, plot_dir, label, monocular=False):
    ...
    ax.legend()
    plt.savefig(os.path.join(plot_dir, "evo_2dplot_{}_seq{}.png".format(str(label), len(poses_gt))), dpi=90)
    plt.close()
...
def eval_ate(frames, kf_ids, save_dir, iterations, final=False, monocular=False):
    ...
    Log(f"It: {iterations} RMSE ATE \[m]", ate, tag="Eval")
    if wandb.run is not None:
        wandb.log({"frame_idx": latest_frame_idx, "ate": ate}, step=iterations)
    return ate
