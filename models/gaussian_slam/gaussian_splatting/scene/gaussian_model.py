...
from models.gaussian_slam.gaussian_splatting.utils.general_utils import (
    build_rotation,
...
from models.gaussian_slam.gaussian_splatting.utils.graphics_utils import BasicPointCloud, getWorld2View2
from models.gaussian_slam.gaussian_splatting.utils.sh_utils import RGB2SH
from models.gaussian_slam.gaussian_splatting.utils.system_utils import mkdir_p
...
class GaussianModel:
    ...
    def construct_list_of_attributes(self, include_score=False):
        l = ["x", "y", "z", "nx", "ny", "nz"]
    ...
    def save_ply(self, path, scores=None):
        ...
        if scores is None:
            ...
            dtype_full = [
                (attribute, "f4") for attribute in self.construct_list_of_attributes(include_score=True)
            ]
            elements = np.empty(xyz.shape[0], dtype=dtype_full)

            attributes = np.concatenate(
                (xyz, normals, f_dc, f_rest, opacities, scale, rotation, scores), axis=1
            )
    ...
    def reset_opacity(self, mask=None):
        if mask is None:
            ...
            opacities_new = inverse_sigmoid(torch.ones_like(self.get_opacity) * 0.01)
            opacities_old = inverse_sigmoid(self.get_opacity)
            opacities_new = torch.where(mask[:, None], opacities_new, opacities_old)
